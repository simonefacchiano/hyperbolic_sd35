...
class CLIPBaseline(nn.Module):
    ...
    def encode_text(self, tokens: list[torch.Tensor], project: bool, return_activations: bool = False):
        """
        Args:
        ...
        text_feats = self.textual(tokens, return_activations=return_activations)
...
class MERU(CLIPBaseline):
    ...
    def encode_text(self, tokens: list[torch.Tensor], project: bool, return_activations: bool = False):
        ...
        # Get Euclidean features from the encoder (without L2 normalization).
        text_feats = super().encode_text(tokens, project=False, return_activations=return_activations)
    ...
    def hyperbolic_projection(self, text_feats: torch.Tensor):
        #text_feats = text_feats * self.textual_alpha.exp()
        with torch.autocast(self.device.type, dtype=torch.float32):
            text_feats = L.exp_map0(text_feats, self.curv.exp())
        return text_feats

    def get_textual_alpha(self):
        return self.textual_alpha.item()
    
    def reverse_hyperbolic_projection(self, hyperbolic_feats: torch.Tensor):
        with torch.autocast(self.device.type, dtype=torch.float32):
            text_feats = L.log_map0(hyperbolic_feats, self.curv.exp())
        #text_feats = text_feats / self.textual_alpha.exp()
        return text_feats
