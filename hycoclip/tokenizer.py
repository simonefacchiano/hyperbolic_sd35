...
class Tokenizer:
    ...
    def __call__(self, text: str | list[str], max_length: int = None, pad_to_max: bool = True) -> list[torch.IntTensor]:
        """
        Returns the tokenized representation of given input string(s).

        Args:
            text: An input string or list of strings to tokenize.
            max_length: If set, pad/truncate each token tensor to this length.
            pad_to_max: If True, pad to max_length. If False, only truncate.
        ...
        for text in text_list:
            ...
            eot_token = self.encoder["<|endoftext|>"]
            bpe_tokens = [sot_token, *bpe_tokens, eot_token]
            toks_tensor = torch.IntTensor(bpe_tokens)

            # Handle max_length: pad or truncate as needed
            if max_length is not None:
                if len(toks_tensor) > max_length:
                    toks_tensor = toks_tensor[:max_length]
                elif pad_to_max and len(toks_tensor) < max_length:
                    padding = torch.full(
                        (max_length - len(toks_tensor),), eot_token, dtype=toks_tensor.dtype
                    )
                    toks_tensor = torch.cat([toks_tensor, padding])

            token_tensors.append(toks_tensor)
