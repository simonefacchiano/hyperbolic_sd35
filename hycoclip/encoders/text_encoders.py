...
class _TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int):
        ...
        self.mlp = MLPWithGelu(d_model)
        self.ln_2 = nn.LayerNorm(d_model)
        self._activation_records = {}

    def forward(self, x: torch.Tensor, attn_mask: torch.Tensor | None = None, return_gelu: bool = False):
        lx = self.ln_1(x)
        ax = self.attn(lx, lx, lx, need_weights=False, attn_mask=attn_mask)[0]
        x = x + ax
        mlp_out, gelu_out = self.mlp(self.ln_2(x))
        x = x + mlp_out
        if return_gelu:
            return x, gelu_out
        return x
...
class TransformerTextEncoder(nn.Module):
    ...
    def __init__(
        ...
        for block in self.resblocks:
            ...
            nn.init.normal_(block.mlp.c_fc.weight, std=(2 * self.width) ** -0.5)
            nn.init.normal_(block.mlp.c_proj.weight, std=out_proj_std)

        self.text_block_activations = []

    def forward(self, text_tokens: torch.Tensor, return_activations: bool = False) -> torch.Tensor:
        """
        ...
        for block in self.resblocks:
            if not return_activations:
                ...
                if self.grad_checkpointing and self.training:
                    # shape: (context_length, batch_size, width)
                    textual_features, activations = checkpoint(block, textual_features, _attn_mask, True)

                else:
                    textual_features, activations = block(textual_features, _attn_mask, True)
                self.text_block_activations.append(activations)
